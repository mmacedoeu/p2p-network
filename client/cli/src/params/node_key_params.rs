...
use ac_network::{config::identity::ed25519, config::NodeKeyConfig};
use ap_core::H256;
...
/// Parse a Ed25519 secret key from a hex string into a `ac_network::Secret`.
fn parse_ed25519_secret(hex: &str) -> error::Result<ac_network::config::Ed25519Secret> {
	H256::from_str(&hex)
		.map_err(invalid_node_key)
		.and_then(|bytes| {
			ed25519::SecretKey::from_bytes(bytes)
				.map(ac_network::config::Secret::Input)
				.map_err(invalid_node_key)
		})
}
...
mod tests {
	use super::*;
	use ac_network::config::identity::{ed25519, Keypair};
	use std::fs;
		...
		fn secret_input(net_config_dir: &PathBuf) -> error::Result<()> {
			NodeKeyType::variants().iter().try_for_each(|t| {
				...
				params.node_key(net_config_dir).and_then(|c| match c {
					NodeKeyConfig::Ed25519(ac_network::config::Secret::Input(ref ski))
			...
			with_def_params(|params| {
				...
				params
					...
					.and_then(move |c| match c {
						NodeKeyConfig::Ed25519(ac_network::config::Secret::File(ref f))
