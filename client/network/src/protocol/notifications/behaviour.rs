...
pub struct Notifications {
	...
	peerset: ac_peerset::Peerset,

	/// List of peers in our state.
	peers: FnvHashMap<(PeerId, ac_peerset::SetId), PeerState>,
	...
	delays: stream::FuturesUnordered<Pin<Box<dyn Future<Output = (DelayId, PeerId, ac_peerset::SetId)> + Send>>>,
	...
	next_incoming_index: ac_peerset::IncomingIndex,
...
struct IncomingPeer {
	...
	incoming_id: ac_peerset::IncomingIndex,
}
...
impl Notifications {
	...
	pub fn new(
		...
		Notifications {
			...
			next_incoming_index: ac_peerset::IncomingIndex(0),
			events: VecDeque::new(),
		}
	}
	...
	pub fn is_open(&self, peer_id: &PeerId, set_id: ac_peerset::SetId) -> bool {
		self.peers.get(&(peer_id.clone(), set_id)).map(|p| p.is_open()).unwrap_or(false)
	}

	/// Disconnects the given peer if we are connected to it.
	pub fn disconnect_peer(&mut self, peer_id: &PeerId, set_id: ac_peerset::SetId) {
	...
	fn disconnect_peer_inner(
		...
		match mem::replace(entry.get_mut(), PeerState::Poisoned) {
			...
			PeerState::DisabledPendingEnable {
				...
				self.peerset.dropped(set_id, peer_id.clone(), ac_peerset::DropReason::Unknown);
	...
	pub fn requested_peers<'a>(&'a self, set_id: ac_peerset::SetId) -> impl Iterator<Item = &'a PeerId> + 'a {
		self.peers.iter()
	...
	fn peerset_report_connect(&mut self, peer_id: PeerId, set_id: ac_peerset::SetId) {
	...
	fn peerset_report_disconnect(&mut self, peer_id: PeerId, set_id: ac_peerset::SetId) {
	...
	/// request from a peer.
	fn peerset_report_accept(&mut self, index: ac_peerset::IncomingIndex) {
		...
		if !incoming.alive {
			...
			match self.peers.get_mut(&(incoming.peer_id.clone(), incoming.set_id)) {
				...
				_ => {
					...
					self.peerset.dropped(incoming.set_id, incoming.peer_id, ac_peerset::DropReason::Unknown);
				},
			}
			return
		}
	...
	fn peerset_report_reject(&mut self, index: ac_peerset::IncomingIndex) {
		let incoming = if let Some(pos) = self.incoming.iter().position(|i| i.incoming_id == index) {
			self.incoming.remove(pos)
		} else {
...
impl NetworkBehaviour for Notifications {
	...
	fn inject_connection_established(&mut self, peer_id: &PeerId, conn: &ConnectionId, endpoint: &ConnectedPoint) {
		for set_id in (0..self.notif_protocols.len()).map(ac_peerset::SetId::from) {
	...
	fn inject_event(
		...
		match event {
			NotifsHandlerOut::OpenDesiredByRemote { protocol_index } => {
				let set_id = ac_peerset::SetId::from(protocol_index);
							...
							.next()
						{
							...
							*entry.into_mut() = PeerState::Enabled { connections };
							...
							if !connections.iter().any(|(_, s)| matches!(s, ConnectionState::Opening)) {
								...
								self.peerset.dropped(set_id, source.clone(), ac_peerset::DropReason::Refused);
								*entry.into_mut() = PeerState::Disabled {
	...
	> {
		...
		loop {
			match futures::Stream::poll_next(Pin::new(&mut self.peerset), cx) {
				Poll::Ready(Some(ac_peerset::Message::Accept(index))) => {
					self.peerset_report_accept(index);
				}
				Poll::Ready(Some(ac_peerset::Message::Reject(index))) => {
					self.peerset_report_reject(index);
				}
				Poll::Ready(Some(ac_peerset::Message::Connect { peer_id, set_id, .. })) => {
					self.peerset_report_connect(peer_id, set_id);
				}
				Poll::Ready(Some(ac_peerset::Message::Drop { peer_id, set_id, .. })) => {
