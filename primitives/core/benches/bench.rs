...
use ap_core::crypto::Pair as _;
use ap_core::hashing::{twox_128, blake2_128};
...
fn bench_ed25519(c: &mut Criterion) {
	c.bench_function_over_inputs("signing - ed25519", |b, &msg_size| {
		let msg = (0..msg_size)
			...
			.collect::<Vec<_>>();
		let key = ap_core::ed25519::Pair::generate().0;
		b.iter(|| key.sign(&msg))
	...
	c.bench_function_over_inputs("verifying - ed25519", |b, &msg_size| {
		...
		b.iter(|| ap_core::ed25519::Pair::verify(&sig, &msg, &public))
	}, vec![32, 1024, 1024 * 1024]);
}
...
fn bench_sr25519(c: &mut Criterion) {
	c.bench_function_over_inputs("signing - sr25519", |b, &msg_size| {
		let msg = (0..msg_size)
			.map(|_| rand::random::<u8>())
			.collect::<Vec<_>>();
		let key = ap_core::sr25519::Pair::generate().0;
		b.iter(|| key.sign(&msg))
	}, vec![32, 1024, 1024 * 1024]);
	...
	c.bench_function_over_inputs("verifying - sr25519", |b, &msg_size| {
		...
		let sig = key.sign(&msg);
		let public = key.public();
		b.iter(|| ap_core::sr25519::Pair::verify(&sig, &msg, &public))
	}, vec![32, 1024, 1024 * 1024]);
}
...
fn bench_ecdsa(c: &mut Criterion) {
	c.bench_function_over_inputs("signing - ecdsa", |b, &msg_size| {
		let msg = (0..msg_size)
			.map(|_| rand::random::<u8>())
			.collect::<Vec<_>>();
		let key = ap_core::ecdsa::Pair::generate().0;
		b.iter(|| key.sign(&msg))
	}, vec![32, 1024, 1024 * 1024]);
	...
	c.bench_function_over_inputs("verifying - ecdsa", |b, &msg_size| {
		...
		let sig = key.sign(&msg);
		let public = key.public();
		b.iter(|| ap_core::ecdsa::Pair::verify(&sig, &msg, &public))
	}, vec![32, 1024, 1024 * 1024]);
}
