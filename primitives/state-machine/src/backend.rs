...
use sp_std::vec::Vec;
#[cfg(feature = "std")]
use ap_core::traits::RuntimeCode;
...
impl<H: Hasher, KF: ap_trie::KeyFunction<H>> Consolidate for ap_trie::GenericMemoryDB<H, KF> {
	fn consolidate(&mut self, other: Self) {
		ap_trie::GenericMemoryDB::consolidate(self, other)
	}
}

/// Insert input pairs into memory db.
#[cfg(test)]
pub(crate) fn insert_into_memory_db<H, I>(mdb: &mut ap_trie::MemoryDB<H>, input: I) -> Option<H::Out>
	where
		H: Hasher,
		I: IntoIterator<Item=(StorageKey, StorageValue)>,
{
	use ap_trie::{TrieMut, trie_types::TrieDBMut};
...
#[cfg(feature = "std")]
impl<'a, B: Backend<H>, H: Hasher> ap_core::traits::FetchRuntimeCode for
